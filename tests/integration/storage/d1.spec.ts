...
describe("d1", () => {
  describe("create entry", () => {
    ...
    it("should return entry with id", async () => {
      expect(body.data.results[0]).toHaveProperty("id", id);
    });

    it("should return entry with data", async () => {
      expect(body.data.results[0]).toHaveProperty("data", "my test data");
    });

    it("should return entry with created_at", async () => {
      expect(body.data.results[0]).toHaveProperty("created_at");
    });

    it("should return entry with updated_at", async () => {
      expect(body.data.results[0]).toHaveProperty("updated_at");
    });
  });
  ...
  describe("update entry", () => {
    ...
    it("should return entry with data", async () => {
      expect(body.data.results[0]).toHaveProperty("data", "my new test data");
    });
